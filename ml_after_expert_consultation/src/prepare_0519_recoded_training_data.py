...
DEFAULT_OUTPUT = "data/processed/chs_training_0519_recoded_expert_consultation.csv"
DEFAULT_REPORT_DIR = "ml_after_expert_consultation/reports/0519_recode"
PHQ_SOURCE_COLUMNS = [f"mtb_07{suffix}1" for suffix in "abcdefghi"]
TARGET_COLUMN = "mh_PHQ_S"
HOUSEHOLD_SOURCE_COLUMN = "mbhld_co"
...
def add_household_features(processed: pd.DataFrame, raw: pd.DataFrame) -> None:
    household_size = numeric(raw[HOUSEHOLD_SOURCE_COLUMN])
    processed["single_person_household"] = np.where(
        household_size.eq(1),
        1.0,
        np.where(household_size.isna(), np.nan, 0.0),
    )


def prepare_dataset(raw: pd.DataFrame, min_age: int, max_age: int) -> tuple[pd.DataFrame, dict[str, Any]]:
    missing = [column for column in list(FEATURE_MAP) + [HOUSEHOLD_SOURCE_COLUMN] if column not in raw.columns]
    if missing:
    ...
    add_household_features(processed, raw)
    ...
    metadata = {
        ...
        "excluded_columns": PHQ_SOURCE_COLUMNS
        + [
            HOUSEHOLD_SOURCE_COLUMN,
            "주관적건강기준",
            "본인인지체형",
            "아침식사일수",
            "교육수준",
        ...
        "exclusion_policy": {
            ...
            HOUSEHOLD_SOURCE_COLUMN: "Recoded to single_person_household: 1 for one-person households, 0 for two or more people.",
            "주관적건강기준": "Removed after expert consultation; judged less specific for youth depression.",
            "본인인지체형": "Removed after expert consultation; both thin and overweight self-perception indicators were excluded.",
            "아침식사일수": "Removed after expert consultation; breakfast itself was not considered a core depression indicator.",
            "교육수준": "Removed after expert consultation; treated as contextual rather than a direct depression-related factor.",
        },
    }
...
def validation_report(
    ...
    for column in FEATURE_COLUMNS:
        ...
        if column != "age" and not values.issubset({0, 1, 0.0, 1.0}):
...
def write_mapping_table(report_dir: Path) -> None:
    ...
    rows.extend(
        [
            {
                "source_column": HOUSEHOLD_SOURCE_COLUMN,
                "processed_column": "single_person_household",
                "note": "1 for one-person households; 0 for households with two or more people; missing stays missing.",
            },
            {
