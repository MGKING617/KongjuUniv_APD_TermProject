...
from ml_after_expert_consultation.src.train_model import (  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="ml_after_expert_consultation/reports/benchmark", help="Directory for benchmark outputs.")
    ...
    parser.add_argument(
        ...
        help="Fixed model probability threshold. Defaults to the prior LightGBM threshold rounded to 0.59.",
    )
    parser.add_argument(
        "--drop-columns",
...
def cross_validate(
    ...
    for fold_index, (train_index, valid_index) in enumerate(splitter.split(x, y), start=1):
        ...
        metrics = evaluate(y_valid, probabilities, decision_threshold)
...
def write_report(metrics: pd.DataFrame, metadata: dict[str, Any], output_dir: Path, cv_frame: pd.DataFrame | None) -> None:
    ...
    if threshold_strategy == "fixed_decision_threshold":
        threshold_policy = "전문가 상담 후 비교 실험에서는 기존 LightGBM 기준 임계값을 반올림한 0.59를 모든 모델 평가에 고정 적용했습니다."
    elif threshold_strategy == "target_recall" and pd.notna(target_recall):
        threshold_policy = f"검증 세트에서 positive recall {float(target_recall):.0%} 이상을 먼저 만족하고, 그 안에서 precision이 가장 높은 임계값을 선택했습니다."
    else:
        threshold_policy = "검증 세트에서 positive F1이 가장 높은 임계값을 선택했습니다."
    lightgbm_note = ""
...
def main() -> None:
    ...
    for spec in specs:
        ...
        validation_metrics = evaluate(y_valid, validation_probabilities, args.fixed_decision_threshold)
        threshold_selection = {
            "threshold": float(args.fixed_decision_threshold),
            "precision": validation_metrics["positive_precision"],
            "recall": validation_metrics["positive_recall"],
            "f1": validation_metrics["positive_f1"],
        ...
        cv_result = cross_validate(spec, x, y, args.cv_folds, args.random_state, selected_threshold)
        ...
        json.dumps(
            {
                ...
                "threshold_strategy": "fixed_decision_threshold",
                "decision_threshold": float(args.fixed_decision_threshold),
                "target_recall": None,
